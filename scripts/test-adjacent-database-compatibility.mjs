...
async function createOwnerState(owner, installation, paths, value) {
  ...
  if (isPreGovernanceDatabaseProduct(installation)) {
    if (owner === "hub") {
      await run(
...
async function writeReadOwner(owner, installation, paths, value, legacyAllowed = false) {
  await preparePhasePaths(paths);
  if (isPreGovernanceDatabaseProduct(installation) && legacyAllowed) {
    if (owner === "hub") {
      await run(
        installation.executable,
        ["access", "create", "--database", paths.database, "--label", value, "--json"],
...
function isPreGovernanceDatabaseProduct(installation) {
  return compareStableVersions(installation.version, "0.4.0") < 0;
}
...
function candidateMigrationDelta(candidate, baseline) {
  if (!candidate?.migrations) return [];
  if (!baseline?.migrations) return candidate.migrations;
