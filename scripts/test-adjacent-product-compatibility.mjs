...
async function runHub(bin, root, assertions, requestedPort) {
  ...
  try {
    ...
    assertions.push({
      ...
      status: "passed",
      detail: "packaged Hub created a scoped runtime enrollment token",
    });
...
async function runTui(bin, root, assertions) {
  const env = await preparePhaseEnv(root);
  const workspace = join(root, "compat-workspace");
  await mkdir(workspace, { recursive: true });
  const firstResult = await command(bin, ["__compat-product", "first", "--json"], {
    cwd: workspace,
    env,
    timeout: 180_000,
  });
  ...
  assertions.push(
    ...
    },
    {
      ...
      status: "passed",
      detail: "TUI created and reread a daemon-managed session snapshot",
    },
    {
...
async function createOrGetHubSession(hubBin, root, env, databasePath, sessionId) {
  const args = ["__compat-product", "session-create", "--database", databasePath, "--json"];
  if (sessionId) args.push("--session", sessionId);
  const result = await command(hubBin, args, { cwd: root, env, timeout: 30_000 });
  return JSON.parse(result.stdout);
}
...
export async function runPhase(spec, installations, root, runtime = DEFAULT_PHASE_RUNTIME) {
  ...
  try {
    ...
    if (spec.actor === "hub") {
      const hub = await runtime.runHub(actor.bin, phaseRoot, details);
      ...
      const createdSession = await runtime.createOrGetHubSession(
        actor.bin,
        phaseRoot,
        hub.env,
        hub.databasePath,
      ...
      if (typeof createdSession?.session?.sessionId !== "string")
        throw new Error("Hub runtime session create failed");
      await runtime.stopChild(hubProcess, hubIdentity);
      hubProcess = undefined;
      hubIdentity = undefined;
      const restarted = await runtime.runHub(actor.bin, phaseRoot, details, hub.port);
      ...
      const rereadSession = await runtime.createOrGetHubSession(
        actor.bin,
        phaseRoot,
        restarted.env,
        restarted.databasePath,
      ...
      if (rereadSession?.session?.sessionId !== createdSession.session.sessionId)
        throw new Error("Hub runtime session get failed after reconnect");
      assertions.push(
        passedAssertion(
          "reconnect",
      ...
      await runtime.prepareDaemonWorkspace(peer.bin, phaseRoot, details);
      daemonStarted = await runtime.runDaemon(peer.bin, phaseRoot, details);
      const probe = await runtime.runTui(actor.bin, phaseRoot, details);
...
export async function buildMatrix({
  ...
  try {
    ...
    try {
      ...
      await runTui(installs.candidate.tui.bin, sanityRoot, sanityAssertions);
      sanityAssertions.push(
        passedAssertion(
          "local-rpc-status",
