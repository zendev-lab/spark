...
for (const runId of expectedRunIds) {
  ...
  const result = spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        CI: "1",
        SPARK_SCRIPTED_PROVIDER_CE: "1",
        SPARK_SCRIPTED_PROVIDER_CE_RUN_ID: runId,
      },
      encoding: "utf8",
  ...
  samples.push({
    ...
    ...(runnerPassed ? {} : { failure: processFailure ?? "scripted provider CE runner failed" }),
  });
...
const report = {
  schema: "spark.scripted-provider-ce/v1",
  generatedAt: new Date().toISOString(),
  sourceSha:
    process.env.SPARK_SCRIPTED_PROVIDER_CE_SOURCE_SHA?.trim() ||
    process.env.GITHUB_SHA?.trim() ||
    undefined,
  testedSha: process.env.GITHUB_SHA?.trim() || undefined,
  configuration: {
    ...configuration,
...
function parseConfiguration(
...
): ScriptedProviderCeConfiguration {
  const normalizedArgs = args[0] === "--" ? args.slice(1) : args;
  if (normalizedArgs.includes("--help") || normalizedArgs.includes("-h")) {
    process.stdout.write(
      [
        "Usage: node --experimental-strip-types scripts/run-scripted-provider-ce.mts [options]",
        "",
        "Options:",
        "  --runs <1-50>",
  ...
  for (let index = 0; index < normalizedArgs.length; index += 1) {
    ...
    if (!known.has(name)) throw new Error(`Unknown scripted provider CE option: ${name}`);
  ...
  const outputDir = resolve(
    ...
    values.get("--output-dir") ??
      env.SPARK_SCRIPTED_PROVIDER_CE_OUTPUT_DIR ??
      "reports/scripted-provider-ce",
  );
  return {
    runs: parseInteger(
      values.get("--runs") ?? env.SPARK_SCRIPTED_PROVIDER_CE_RUNS ?? "8",
      "runs",
      1,
      50,
    ),
    ...
    maxFailureRate: parseRate(
      values.get("--max-failure-rate") ?? env.SPARK_SCRIPTED_PROVIDER_CE_MAX_FAILURE_RATE ?? "0",
      "max-failure-rate",
    ),
    maxDurationP95Ms: parseInteger(
      values.get("--max-duration-p95-ms") ??
        env.SPARK_SCRIPTED_PROVIDER_CE_MAX_DURATION_P95_MS ??
        "10000",
      "max-duration-p95-ms",
      1,
      600_000,
    ...
    runTimeoutMs: parseInteger(
      values.get("--run-timeout-ms") ?? env.SPARK_SCRIPTED_PROVIDER_CE_RUN_TIMEOUT_MS ?? "60000",
      "run-timeout-ms",
      1_000,
      900_000,
    ),
      ...
      for (const rawAssertion of assertionResults) {
        ...
        const ancestorTitles = Array.isArray(assertion?.ancestorTitles)
          ? assertion.ancestorTitles.flatMap((value) => {
              const ancestor = stringValue(value);
...
function renderAssertionFailure(
  ...
  const messages = Array.isArray(assertion?.failureMessages)
    ...
    : [];
  return truncate(messages.join("\n") || `Vitest assertion status=${status}`, 6_000);
}
...
function renderMarkdownReport(report: {
  ...
  sourceSha?: string;
  testedSha?: string;
  configuration: ScriptedProviderCeConfiguration & {
    testFile: string;
  ...
  const lines = [
    ...
    ...(report.sourceSha ? [`**Source commit:** \`${report.sourceSha}\``] : []),
    ...(report.testedSha ? [`**Tested commit:** \`${report.testedSha}\``] : []),
    ...
    `**Transport:** ${report.configuration.transportPolicy}`,
    "",
  ...
  lines.push(
    ...
    `- Test file: ${report.configuration.testFile}`,
    "",
  );
  return `${lines.join("\n")}\n`;
}
