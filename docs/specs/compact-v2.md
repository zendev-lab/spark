...
`/compact` is a daemon-owned durable operation against the current canonical Session transcript. Admission freezes the open Session incarnation, and transcript binding and completion compare that fence before mutation is accepted. Cancellation and timeout abort the Smart-summary provider signal and must not persist a fallback compaction. The final cancellation check and scheduler fence occur immediately before atomic transcript replacement; after that commit point begins, late cancellation and timeout cannot publish a contradictory terminal state, and the invocation drains to its real success or failure. The same-version TUI submits the typed `session.compact` request, waits for its invocation to reach a terminal state, then refreshes the transcript from `session.snapshot`. It must not compact the bounded visible projection or create a parallel `compact-*` transcript.
...
Immediately before opening a provider stream, Spark meters the assembled provider input rather than the bounded UI projection. The meter includes the current system instructions, every lowered provider message (including hidden runtime envelopes), and active tool descriptions and schemas. The requested output budget remains visible to diagnostics but is not added to this final hard bound because the provider adapter clamps it to the capacity left after input and its safety allowance. A nonzero provider-reported prefix may replace the local estimate for message history, but the current system and tool components are always added. A compaction summary resets reported-prefix accounting, and usage attached to assistant messages retained from before that boundary is removed during canonical replay projection. A preflight overflow enters the same compact-and-retry path as a provider-reported context overflow.
