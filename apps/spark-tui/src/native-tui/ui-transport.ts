...
import type { SparkHostUiTransport } from "@zendev-lab/spark-host/types";
...
export function createSparkNativeUiTransport(
  ...
  return {
    ...
    input: (title, defaultValue) => app.input(title, defaultValue),
    secret: (title) => app.secret(title),
    select: (title, options) => app.select(title, options),
