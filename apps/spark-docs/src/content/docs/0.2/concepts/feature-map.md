...
description: See every Spark capability by product surface, state owner, and
  user intent instead of by slash-command count.
slug: 0.2/concepts/feature-map
---
...
[surfaces and ownership](/0.2/concepts/surfaces/) and the [CLI reference](/0.2/reference/cli/).
...
[runs and sessions](/0.2/guides/runs-and-sessions/) for foreground, background,
attach, resume, and cancellation.

## 2. Interactive design: Cockpit and TUI

* Use the [TUI](/0.2/guides/tui/) for fast local conversation, Plan/Implement,
  steering, model selection, and the current session inspector.
* Use [Cockpit](/0.2/guides/cockpit/) for workspace overview, conversations,
...
See the complete, profile-aware [agent tool catalog](/0.2/reference/tools/).
...
Start with [plan and implement](/0.2/guides/plan-and-implement/), then read
[long-running automation](/0.2/guides/automation/).
...
See [collaboration and channels](/0.2/guides/collaboration/) and
[Side Threads](/0.2/guides/side-threads/).
...
Use [configuration and paths](/0.2/reference/configuration-and-paths/) before
changing runtime storage, and [troubleshooting](/0.2/troubleshooting/) when a
