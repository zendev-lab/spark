...
template: splash
hero:
  tagline: Describe a goal, turn it into verifiable tasks, and supervise the
    implementation from the terminal or Cockpit.
  actions:
    - text: Get started
      link: /0.2/getting-started/
      icon: right-arrow
    - text: CLI reference
      link: /0.2/reference/cli/
      icon: right-arrow
      variant: minimal
sidebar:
  order: 1
...
Start with [installation and your first run](/0.2/getting-started/), then
[plan and implement a change](/0.2/guides/plan-and-implement/). Learn
[the complete feature map](/0.2/concepts/feature-map/) when you want to see how
CLI, daemon, TUI, Cockpit, tools, automation, and collaboration fit together.
Read [surface ownership](/0.2/concepts/surfaces/) only when you need to automate,
