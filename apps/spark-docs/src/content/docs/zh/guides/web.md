...
兼容应用会恢复 Spark LLM 与 Cue 插件，并将内置的 `spark-cue` Skill 挂入
DSH Skill 目录。它会处理明文 HTTP UUID 和远程 credential onboarding，并在
DSH 将完整 transcript 载入内存前拒绝过大的冷历史文件。对于可以安全读取的
