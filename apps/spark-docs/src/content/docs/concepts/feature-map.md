...
| --- | --- |
| `apps/spark-cli`, `spark-tui`, `spark-daemon`, `apps/spark-hub` (Hub compatibility path) | Executable dispatcher and presentation/runtime hosts |
...
| `packages/spark-hub-*` | Hub-private database, coordination, and localization implementation under compatibility paths |
