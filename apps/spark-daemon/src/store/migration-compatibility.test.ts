...
function changedManifest(change: (manifest: Record<string, any>, root: string) => void): string {
  const root = mkdtempSync(join(tmpdir(), "spark-daemon-manifest-"));
  roots.push(root);
  const path = join(root, "manifest.json");
  const sourceRoot = join(import.meta.dirname, "migrations");
  const manifest = JSON.parse(readFileSync(join(sourceRoot, "manifest.json"), "utf8")) as Record<
    string,
    any
  >;
  copyFileSync(
