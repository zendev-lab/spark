...
import type { SparkDriverStore } from "./drivers.ts";
import type { SparkInvocationStore } from "./invocations.ts";

export interface SparkDriverStoreContractHarness {
  db: DatabaseSync;
  invocations: SparkInvocationStore;
  drivers: SparkDriverStore;
  close(): void;
}

/**
 * Reusable behavioral contract for daemon-owned driver stores.
 *
 * Implementations provide only a fresh harness. The suite deliberately observes
 * public store transitions and persisted records rather than private SQL helpers.
 */
export function runSparkDriverStoreContract(
  createHarness: () => SparkDriverStoreContractHarness,
): void {
  describe("SparkDriverStore contract", () => {
    it("DRV-ADMISSION-001 coalesces an overdue wake while the owner session is busy", () => {
    ...
    it("DRV-ADMISSION-002 does not let one busy owner starve another owner's due driver", () => {
      const harness = createHarness();
      try {
        harness.drivers.start({
    ...
    it("DRV-GENERATION-002 clears retry attempts when a recovered tick explicitly schedules success", () => {
      const harness = createHarness();
      try {
        harness.drivers.start({
    ...
    it("DRV-GENERATION-003 advances generation before materializing policy continuations and retries", () => {
      const harness = createHarness();
      try {
    ...
    it("DRV-RECOVERY-001 keeps a running wake attached to the same invocation across resume", () => {
      const harness = createHarness();
      try {
    ...
    it("DRV-RECOVERY-002 reconciles a terminal invocation left beside an unsettled running wake", () => {
      const harness = createHarness();
      try {
    ...
    it("DRV-FAILURE-001 applies backoff and fail-closed terminal policies", () => {
      const harness = createHarness();
      try {
    ...
    it("DRV-FAILURE-002 uses capped retry sequences for foreground and workflow drivers", () => {
    ...
    it("DRV-CONTINUITY-001 builds fresh ticks against a hidden reset session while retaining owner state", () => {
      const harness = createHarness();
      try {
        harness.drivers.start({
          driverId: "loop-fresh",
    ...
    it("DRV-WAKE-001 consumes a manual wake prompt exactly once and retains the base driver prompt", () => {
      const harness = createHarness();
      try {
        harness.drivers.start({
    ...
    it("DRV-CONTINUITY-002 garbage-collects expired fresh sessions and retains failed removals", async () => {
      const harness = createHarness();
      try {
        harness.drivers.start({
    ...
    it("DRV-POLICY-001 uses capability success policies without spinning loop or workflow", () => {
    ...
    it("DRV-LANE-001 atomically replaces the foreground lane while preserving workflow background work", () => {
      const harness = createHarness();
      try {
        harness.drivers.start({
          driverId: "workflow",
    ...
    it("DRV-LANE-002 makes wake replace the foreground lane", () => {
      const harness = createHarness();
      try {
        harness.drivers.start({
          driverId: "goal-old",
    ...
    it("DRV-CANCEL-001 requests cancellation for a running tick when its driver is stopped", () => {
      const harness = createHarness();
      try {
    ...
    it("DRV-CANCEL-002 lets the current tick stop its driver without aborting its own invocation", () => {
      const harness = createHarness();
      try {
    ...
    it("DRV-CANCEL-003 cancels the current tick when the same driver is started or restarted", () => {
      const harness = createHarness();
      try {
...
function runningTick(
  harness: SparkDriverStoreContractHarness,
  driverId: string,
