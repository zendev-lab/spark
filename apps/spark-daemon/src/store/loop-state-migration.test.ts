...
describe("legacy autonomous loop migration", () => {
  it("DRV-STARTUP-004 imports legacy cadence once, strips frontend runtime fields, and repairs a missing wake", async () => {
    ...
    try {
      const report = await migrateLegacyLoopState({
        db,
        loopStore: loops,
      ...
      expect(loops.require("goal-migrate")).toMatchObject({
        status: "retry_wait",
        attempt: 1,
        dueAt: "2026-07-23T00:01:00.000Z",
      });
      expect(loops.require("loop-migrate").status).toBe("stopped");
      expect(loops.get("implement:session-migrate")).toBeUndefined();
