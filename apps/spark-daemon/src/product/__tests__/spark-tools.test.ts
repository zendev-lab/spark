...
import { SparkWorkflowRunManagerController } from "../policy/spark-workflow-run-manager.ts";
import { JsonStoreFormatError } from "../policy/json-store.ts";
import type { SparkToolContext } from "../policy/spark-tool-registration.ts";
import type { SparkDaemonLoopControl } from "../policy/spark-daemon-loop-client.ts";
import type { SparkDaemonReproControl } from "../policy/spark-daemon-repro-client.ts";
import type { SparkTaskClaimDaemonClient } from "../policy/spark-task-claim-daemon-client.ts";
import {
...
} from "../policy/session-todos.ts";
import { renderSessionTodoContext } from "../policy/spark-session-todo-context.ts";
import {
  normalizeSparkStatusFormat,
  normalizeSparkStatusLimit,
  normalizeSparkStatusView,
} from "../policy/spark-status.ts";
import {
...
} from "../policy/background-runs.ts";
import {
  normalizeSparkRunReadyTasksBoolean,
  normalizeSparkRunReadyTasksPositiveInteger,
} from "../policy/spark-run-ready-tasks-tool-registration.ts";
import { normalizeSparkPlanTaskInputs } from "../policy/spark-plan-tasks-tool-registration.ts";
import { normalizeSparkClaimTaskInput } from "../policy/spark-claim-task-tool-registration.ts";
import {
  buildTaskReviewEvidenceContext,
  normalizeSparkFinishTaskInput,
} from "../policy/spark-finish-task-tool-registration.ts";
import {
  ...
  taskReviewDirectory,
} from "../policy/subject-review-store.ts";
import { normalizeSparkTodoOps } from "../policy/spark-todo-tool-registration.ts";
import {
  normalizeEvidenceBoolean,
  normalizeEvidenceLimit,
  normalizeEvidenceRef,
  normalizePositiveInteger,
} from "../policy/evidence-tools.ts";
import {
...
} from "../policy/spark-workflow-runs-tool-registration.ts";
...
} from "../policy/spark-project-tools.ts";
import {
  normalizeSparkStateAction,
  normalizeSparkStateOptionalString,
} from "../policy/spark-state-tool-registration.ts";
import { normalizeTaskKind, normalizeTaskStatus } from "../policy/task-plan-tool.ts";
import { normalizeSparkAskReplayEvidenceRef } from "../policy/spark-ask-tool-registration.ts";
import {
...
type SparkHostApiForTest = Parameters<typeof registerSparkProduct>[0];
...
const evidenceSurfaceNegativeValues = JSON.parse(
  readFileSync(
    join(REPO_ROOT, "test", "fixtures", "evidence-surface", "negative-values.json"),
    "utf8",
  ),
...
async function loadLegacyEvidenceFixture<T>(
  ...
  const fixturePath = join(REPO_ROOT, "test", "fixtures", "legacy-evidence", name);
        ...
        () =>
          normalizeSparkFinishTaskInput({
            evidence: {
              ...
              notes: " Notes ",
              changedFiles: [" apps/spark-daemon/src/product/file.ts "],
...
test("Spark product policy leaves Shift+Tab to the host thinking-level binding", () => {
  const run = registerSparkToolsForTest();
  ...
  try {
    ...
    const finished = await executeSparkTool(tools, "impl_finish_task", ctx, {
      ...
      evidence: {
        ...
        changedFiles: [
          "apps/spark-daemon/src/product/policy/spark-finish-task-tool-registration.ts",
        ],
...
test("Spark product policy exposes canonical tools instead of removed spark_* tools", () => {
  const run = registerSparkToolsForTest();
...
function registerSparkToolsForTest(
  ...
  const pi: SparkHostApiForTest & {
    ...
    loopControl,
    reproControl:
      options.reproControl ??
      ({
        async start() {
          throw new Error("unexpected Repro start in this test");
        },
        async status() {
          return {};
        },
        async stop() {
          throw new Error("unexpected Repro stop in this test");
        },
      } satisfies SparkDaemonReproControl),
