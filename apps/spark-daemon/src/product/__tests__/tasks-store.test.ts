...
} from "../policy/spark-background-subrole-lifecycle.ts";
...
} from "../policy/spark-ready-task-runtime.ts";
import { saveCurrentProjectRef, sparkSessionOwnerKey } from "../policy/session-state.ts";
...
test("workflow run store normalizes legacy completion digests without inventing evidence", async () => {
  ...
  try {
    ...
    const fixturePath = join(
      import.meta.dirname,
      "../../../../../test/fixtures/legacy-evidence/workflow-run-v1-completion-digest.json",
    );
