...
import { loadProviderPlugins, type ProviderPluginLoadResult } from "./provider-plugin-loader.ts";
import {
...
export async function createSparkCliHostServices(
  ...
  const runtime = new SparkHostRuntime({
    ...
    invocationAttempt: options.invocationAttempt,
    invocationRole: options.invocationRole,
    driverAuthority: options.driverAuthority,
  ...
  // Registered before product capabilities so request-scoped prompt state is cleared
  // before any later agent_end handler can enqueue a background turn.
  ...
  const providerLoadResult = await loadProviderPlugins({
    providerApi: providerRegistry,
    providers: options.providers ?? config.providers,
    ...(options.providerImporter ? { importer: options.providerImporter } : {}),
  });
