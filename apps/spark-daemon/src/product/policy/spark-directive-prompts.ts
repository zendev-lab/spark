...
export const MAIN_SESSION_SCHEDULING_FIRST =
  'Keep scheduling and reconciliation on the owner Session. Do not default to session({ action: "spawn"|"fork"|"send" }), assign, or workflow action=run for ordinary Goal/driver ticks. Use those only when the user explicitly requests multi-agent/workflow fan-out, or when a clearly parallelizable slice requires a separately owned execution context.';
