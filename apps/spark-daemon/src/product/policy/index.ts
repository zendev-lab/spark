...
import { registerSparkProductEvents } from "./spark-product-events.ts";
...
export default function registerSparkProductPolicy(pi: SparkProductFacadeApi) {
  ...
  const contextProviders: SparkContextProvider[] = [
    {
      id: "spark.active",
      label: sparkProductContextProviderStrings.label,
      description: sparkProductContextProviderStrings.description,
  ...
  const eventHandlers = registerSparkProductEvents(pi, {
    refreshSparkWidget,
  ...
  const registerSparkTool = (config: SparkRegisteredToolConfig): void => {
    ...
    const copy = sparkProductToolCopy(config.name, config);
    pi.registerTool?.({
      ...config,
      ...copy,
  ...
  const registerSparkImplementationTool = (config: SparkRegisteredToolConfig): void => {
    registeredSparkTools.set(config.name, config);
    pi.registerInternalTool?.({ ...config, ...sparkProductToolCopy(config.name, config) });
  };
