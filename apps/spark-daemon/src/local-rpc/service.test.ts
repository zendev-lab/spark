...
describe("transport-neutral local RPC service", () => {
  ...
  it("projects Role and Skill catalogs without exposing resource paths", async () => {
    ...
    await expect(
      ...
      role: { ref: created.role.ref, modelType: "verification" },
      setting: { modelType: "verification", model: "test/reviewer", source: "project" },
    });
    await expect(
      invokeLocalRpcService(
        "role.model.get",
        { workspaceId: workspace.id, roleRef: created.role.ref },
        { paths, db },
      ),
    ...
    await expect(
      invokeLocalRpcService(
        "role.model.delete",
        { workspaceId: workspace.id, roleRef: created.role.ref, source: "project" },
        { paths, db },
      ),
