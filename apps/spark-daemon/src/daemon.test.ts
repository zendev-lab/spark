...
describe("Spark daemon handleCommand task.start.request", () => {
  ...
  it("DRV-STARTUP-005 keeps production scheduler and channel admission paused when serving fence commit fails", async () => {
    ...
    loops.start({
      loopId: "retry-before-ready",
      binding: { goalId: "retry-before-ready" },
    ...
    try {
      ...
      expect(loops.require("scheduled-before-ready").status).toBe("scheduled");
      expect(loops.require("scheduled-before-ready").lastInvocationId).toBeUndefined();
      expect(loops.require("retry-before-ready")).toMatchObject({
        status: "retry_wait",
        attempt: 2,
      });
      expect(loops.require("retry-before-ready").lastInvocationId).toBeUndefined();
