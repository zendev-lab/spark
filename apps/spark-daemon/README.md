...
## Daemon database compatibility

The daemon owns `src/store/migrations/manifest.json`. Because its historical schema evolution was inline, the manifest records only the honest `legacy-inline-v0` baseline and explicitly declares that numbered historical migrations are unavailable. Its checksum covers the packaged deterministic `legacy-inline-v0.schema.json` descriptor derived from normalized SQLite table/column/index metadata, not a source-file name or an unverifiable migration history. Runtime adoption validates the descriptor as a required schema subset, then records a database-specific canonical `sqlite_master` fingerprint so later unnumbered drift fails closed. Markers also reject unknown/future heads, dirty/failed state, and baseline-checksum drift. The published `spark-daemon __compat-database ... --json` probe uses the same real open/migrate path and is packaged with both manifest and descriptor. Future daemon schema changes must be added as real governed migrations rather than rewriting the legacy baseline.
