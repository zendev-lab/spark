...
server.listen(port, host, () => {
  console.log(`Spark Hub server listening on http://${host}:${port}`);
  if (publicUrl.mode === "fixed") {
    console.log(`Spark Hub public URL: ${publicUrl.publicUrl}`);
  } else if (publicUrl.mode === "proxy") {
    console.log("Spark Hub public URL is derived from its trusted loopback proxy.");
  }
