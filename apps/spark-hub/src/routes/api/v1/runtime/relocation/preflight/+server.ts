...
import {
  hubRuntimeRelocationInstanceId,
  preflightHubRuntimeRelocation,
...
export const POST: RequestHandler = async ({ request, url, locals }) => {
  ...
  if (!instanceId || instanceId !== parsed.data.sourceInstanceId) {
    return errorJson(
      ...
      "Target Hub is not the relocated source instance.",
      409,
      undefined,
      locals.requestId,
    );
  ...
  try {
    const refreshed = preflightHubRuntimeRelocation(parsed.data);
