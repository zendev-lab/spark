...
} from "$lib/server/hub-runtime-session-client";
import {
  controlManagedSideThreadForHub,
...
function sideThreadActionErrorResponse(error: unknown): Response {
  ...
  if (isHubRuntimeSessionNotFoundError(error)) {
...
export const GET: RequestHandler = async ({ locals, params, url }) => {
  try {
    ...
    const snapshot = await getManagedSideThreadSnapshotForHub(params.sessionId, {
      workspaceId,
...
/**
 * Hub is a transport adapter for the daemon-owned Side Thread controller.
...
export const POST: RequestHandler = async ({ locals, params, request }) => {
  try {
    ...
    const result = await controlManagedSideThreadForHub(
      parentSessionId,
      workspaceId,
      async (client) => {
        if (action === "ensure") {
          if (!client.ensureSideThread) throw new HubRuntimeSessionUnavailableError("unavailable");
          return await client.ensureSideThread(parsed as never);
        }
        if (action === "submit") {
          if (!client.submitSideThread) throw new HubRuntimeSessionUnavailableError("unavailable");
          return await client.submitSideThread(parsed as never);
        }
        if (action === "reset") {
          if (!client.resetSideThread) throw new HubRuntimeSessionUnavailableError("unavailable");
          return await client.resetSideThread(parsed as never);
        }
        if (action === "configure") {
          if (!client.configureSideThread)
            throw new HubRuntimeSessionUnavailableError("unavailable");
          return await client.configureSideThread(parsed as never);
        }
        if (!client.handoffSideThread) throw new HubRuntimeSessionUnavailableError("unavailable");
