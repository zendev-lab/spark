import { sparkAgentsHubSource } from "@zendev-lab/spark-protocol/command-sources";
import type { AgentsProductProjection } from "@zendev-lab/spark-hub-coordination/agents-product";

export const agentsHubSource = sparkAgentsHubSource;
...
export function addOptimisticAgentsChatCommand(
  ...
  state.commands.unshift({
    ...
    payloadJson: JSON.stringify({
      kind: "task.start.request",
      title: prompt,
      payload: { prompt, runtimeTaskId, source: agentsHubSource },
    }),
    status: "queued",
...
function mergeQueuedCommand(state: AgentsChatLiveState, event: AgentsChatSerializedEvent): boolean {
  ...
  if (!command || !isAgentsHubServerCommand(parseJson(command.payloadJson))) return false;
...
function isAgentsHubServerCommand(value: unknown): boolean {
  if (!isRecord(value) || !isRecord(value.payload)) return false;
  return value.payload.source === agentsHubSource;
}
