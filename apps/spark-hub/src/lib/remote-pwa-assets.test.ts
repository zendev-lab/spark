...
describe("remote PWA assets", () => {
  it("ships an installable Hub manifest with maskable icons", () => {
  ...
  it("keeps approval and conversation detail surfaces responsive on phone-width viewports", () => {
    ...
    const hubShell = readAppFile("src/lib/shell/HubShell.svelte");
    const hubTopbar = readAppFile("src/lib/shell/HubTopbar.svelte");
    ...
    expect(workbenchLayout).toContain('import HubShell from "$lib/shell/HubShell.svelte"');
    expect(hubShell).toContain("@media (max-width: 900px)");
    expect(hubTopbar).toContain("@media (max-width: 560px)");
  });
});
