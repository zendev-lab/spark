import type { HumanQuestion } from "@zendev-lab/spark-hub-coordination/hub-queries";
import {
...
export const hubCustomAnswerValue = "__spark_hub_custom_answer__";
...
export function humanSingleAnswerWithCustomFallback(
  ...
  if (value === hubCustomAnswerValue || question.type === "freeform" || !question.options?.length) {
    const customText = (value === hubCustomAnswerValue ? customAnswer : selected).trim();
    return {
      values: [],
...
export function humanMultiAnswerWithCustomFallback(
  ...
  const values = [
    ...new Set(
      selected
        .map((value) => value.trim())
        .filter((value) => value && value !== hubCustomAnswerValue),
    ),
  ];
  const answer = answerFromParsedChoice(question, values.join(","));
  const customText = selected.includes(hubCustomAnswerValue) ? customAnswer.trim() : "";
  return {
    ...answer,
    ...(customText ? { customText } : {}),
  };
