...
export type HubChatTurnStatus = "waiting" | "running" | "completed" | "error" | "cancelled";

export type HubChatTranscriptTurn = {
  id: string;
  command: HubChatCommand;
  prompt: string;
  runtimeTaskId: string | null;
  invocations: HubChatInvocation[];
  logs: HubChatLogChunk[];
  status: HubChatTurnStatus;
  answer: string;
  renderSource: string | null;
  currentActivity: string | null;
};

export type HubChatTranscriptLabels = {
...
export const defaultHubChatTranscriptLabels: HubChatTranscriptLabels = {
...
export function buildHubChatTranscriptTurns(
  sourceCommands: HubChatCommand[],
  sourceInvocations: HubChatInvocation[],
  sourceLogs: HubChatLogChunk[],
  labels: HubChatTranscriptLabels = defaultHubChatTranscriptLabels,
) {
  return sourceCommands
    ...
    .map((command): HubChatTranscriptTurn => {
...
export function parseCommandPayload(command: HubChatCommand) {
  try {
...
export function activityKind(log: HubChatLogChunk) {
...
export function latestActivity(logs: HubChatLogChunk[]) {
...
function turnStatus(
  command: HubChatCommand,
  relatedInvocations: HubChatInvocation[],
  relatedLogs: HubChatLogChunk[],
): HubChatTurnStatus {
...
function assistantAnswer(
  status: HubChatTurnStatus,
  logs: HubChatLogChunk[],
  labels: HubChatTranscriptLabels,
  relatedInvocations: HubChatInvocation[] = [],
  command?: HubChatCommand,
) {
  if (status === "error") {
...
function assistantRenderSource(status: HubChatTurnStatus, logs: HubChatLogChunk[]) {
  if (status === "error" || status === "cancelled") return null;
  return orderedAssistantRenderSource(logs) ?? latestReadableRenderSource(logs);
}

export function orderedAssistantOutput(logs: HubChatLogChunk[]) {
  return boundedOutput(orderedAssistantRenderSource(logs));
}

export function orderedAssistantRenderSource(logs: HubChatLogChunk[]) {
  const content = logs
...
function latestReadableOutput(logs: HubChatLogChunk[]) {
  return boundedOutput(latestReadableRenderSource(logs));
}

function latestReadableRenderSource(logs: HubChatLogChunk[]) {
  const content = logs
    .slice()
    .reverse()
...
function compareLogChunks(left: HubChatLogChunk, right: HubChatLogChunk) {
...
export function isStaleRunningInvocation(
  invocation: Pick<HubChatInvocation, "status" | "updatedAt">,
  nowMs = Date.now(),
