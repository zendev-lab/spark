...
export async function handleWorkspaceAccessCliCommand(
  ...
  if (operation !== "create" && operation !== "list" && operation !== "revoke") {
    throw new Error(
      `unknown spark hub workspace access operation: ${command.operation}. Use create, list, or revoke.`,
    );
  }
  const workspaceRef = command.workspaceRef?.trim();
  if (!workspaceRef) {
    throw new Error("spark hub workspace access requires --workspace <id>");
  }
  if (operation === "revoke" && !command.tokenId?.trim()) {
    throw new Error("spark hub workspace access revoke requires --id <token-id>");
  }
  ...
  try {
    ...
    switch (operation) {
      ...
      default: {
        ...
        void _exhaustive;
        throw new Error(`unhandled spark hub workspace access operation: ${command.operation}`);
      }
    }
  } finally {
    db.close();
