...
test("HTTPS Hub controls models and channels over WSS without a daemon socket", async () => {
  const root = await mkdtemp(join(tmpdir(), "spark-remote-model-channel-"));
  const daemonHome = join(root, "daemon-home");
  const hubCache = join(root, "hub-cache");
  const hubArtifacts = join(root, "hub-artifacts");
  const nonexistentSocket = join(root, "hub-host", "daemon.sock");
  ...
  try {
    await Promise.all([
      ...
      mkdir(hubCache, { recursive: true }),
      mkdir(hubArtifacts, { recursive: true }),
    ]);
    migrate(hubDb);
    ...
    const hubWorkspace = createWorkspaceWithLease(hubDb, {
      slug: "remote-model",
    ...
    const daemonWorkspace = registerWorkspace(daemonDb, {
      ...
      serverWorkspaceId: hubWorkspace.id,
    ...
    const owner = createOwnerSession(hubDb, "Owner", null);
    ...
    const client = createHubRuntimeModelChannelClient(hubDb);
    const tls = createTestCertificate(root);
    const requestHandler = createControlRequestHandler(hubDb, client, {
      runtimeId,
    ...
    httpsServer.on("upgrade", (request, socket, head) => {
      const tokenId = authenticateRuntimeToken(hubDb, runtimeId, request.headers.authorization);
      if (!tokenId) {
        socket.destroy();
        return;
      }
    ...
    await waitFor(() => connectedRuntimeSessions(hubDb, runtimeId) === 1);
    ...
    assert.equal(databaseText(hubDb).includes(secretMarker), false);
    assert.equal(databaseText(daemonDb).includes(secretMarker), false);
    assert.equal(directoryText(hubCache).includes(secretMarker), false);
    assert.equal(directoryText(hubArtifacts).includes(secretMarker), false);
    ...
    assert.equal(
      existsSync(join(daemonHome, "workspaces", hubWorkspace.id, "channels", "config.json")),
      false,
    );
    ...
    assert.equal(
      Number(
        hubDb.prepare("SELECT COUNT(*) AS count FROM runtime_ephemeral_secret_audit").get()!.count,
      ),
      4,
    );
    const commandKinds = hubDb
...
function createControlRequestHandler(
  db: DatabaseSync,
  client: ReturnType<typeof createHubRuntimeModelChannelClient>,
  route: {
    runtimeId: string;
    workspaceId: string;
