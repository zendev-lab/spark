...
test("remote Hub controls workspace sessions without a daemon socket", async () => {
  ...
  try {
    ...
    assert.ok(
      listProjectedManagedSessionsForHub(
        { workspaceId: hubWorkspace.id, related: true },
        hubDb,
    ...
    const workspaceTurn = await client.submit({
      ...
      assignment: assignment(workspaceSessionId, "Workspace round one", hubWorkspace.id),
      idempotencyKey: createId("idem"),
    });
    ...
    const secondTurnPromise = client.submit({
      ...
      assignment: assignment(secondSessionId, "Workspace round two", hubWorkspace.id),
      idempotencyKey: createId("idem"),
    });
    const droppedCommand = await waitForHubCommand(hubDb, {
    ...
    bridge.close(1006, "drop accepted command terminal before Hub ingest");
    bridge = connectRuntime(hubDb, context, bindingId);
    bridges.push(bridge);
    ...
    const projectedCursorCount = hubDb
      .prepare(
    ...
    const terminalProjectionCount = hubDb
      .prepare(
        `SELECT COUNT(*) AS count FROM runtime_invocation_event_projections
...
class RuntimeBridge extends EventEmitter implements RuntimeWebSocketConnection {
  ...
  send(data: string): void {
    ...
    if (value.type === "server.command") {
      ...
      void handleCommand(new DaemonToHubSocket(this, command), command, this.context).catch(
...
class DaemonToHubSocket implements ServerSocket {
...
async function waitForHubCommand(
  db: DatabaseSync,
