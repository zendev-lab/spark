...
const env = {
  SPARK_HUB_VAPID_PUBLIC_KEY: "public-key",
  SPARK_HUB_VAPID_PRIVATE_KEY: "private-key",
  SPARK_HUB_VAPID_SUBJECT: "mailto:ops@example.test",
};
...
describe("web push notifications", () => {
  ...
  it("pins the Hub DB per tick and backs off when no subscription is stored", async () => {
    vi.useFakeTimers();
    const root = mkdtempSync(join(tmpdir(), "spark-hub-web-push-dispatch-"));
    roots.push(root);
