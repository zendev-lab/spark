...
describe("package boundaries", () => {
  ...
  it("keeps Hub UI outside lib/server from importing spark-hub-db directly", () => {
    ...
    const violations = uiFiles.filter((file) =>
      /from\s+["']@zendev-lab\/spark-hub-db/u.test(readFileSync(file, "utf8")),
    );
    expect(violations).toEqual([]);
  });

  it("keeps Hub source from bypassing daemon/protocol workspace artifact access", () => {
  ...
  it("keeps artifact fallback out of daemon/local workspace files", () => {
    const sourceFile = ts.createSourceFile(
      "agents-product.ts",
      readFileSync(join(repoRoot, "packages/spark-hub-coordination/src/agents-product.ts"), "utf8"),
  ...
  it("does not import Spark daemon internals from the hub app", () => {
  ...
  it("keeps client session mutations behind daemon local RPC", () => {
    const clientRoots = [
      join(webRoot, "src"),
      join(repoRoot, "packages/spark-hub-coordination/src"),
    ];
