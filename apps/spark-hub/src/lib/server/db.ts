...
interface HubDatabaseState {
  database?: ReturnType<typeof openDatabase>;
  databaseLock?: HubDatabaseLockHandle;
  pinCount: number;
...
const globalScope = globalThis as typeof globalThis & {
  __sparkHubDatabaseState__?: HubDatabaseState;
};
const state = (globalScope.__sparkHubDatabaseState__ ??= {
  pinCount: 0,
...
export function getDatabase(): DatabaseSync {
  if (!state.database) {
    throw new Error(
      "Hub database is closed. Call pinDatabase() or withDatabase() before getDatabase().",
    );
  }
  return state.database;
}
