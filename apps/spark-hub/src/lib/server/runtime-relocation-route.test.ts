...
import { RuntimeRelocationPreflightError } from "@zendev-lab/spark-hub-coordination/runtime-registration";
...
const mocks = vi.hoisted(() => ({
  preflightHubRuntimeRelocation: vi.fn(),
  hubRuntimeRelocationInstanceId: vi.fn(),
}));
...
vi.mock("$lib/server/runtime-relocation", async () => ({
  ...(await vi.importActual("@zendev-lab/spark-hub-coordination/runtime-registration")),
  preflightHubRuntimeRelocation: mocks.preflightHubRuntimeRelocation,
  hubRuntimeRelocationInstanceId: mocks.hubRuntimeRelocationInstanceId,
}));
...
describe("runtime relocation routes", () => {
  ...
  it("returns only non-secret Hub instance metadata", async () => {
    mocks.hubRuntimeRelocationInstanceId.mockReturnValueOnce(instanceId);
  ...
  it("rotates a matching restored runtime and returns same-origin WSS metadata", async () => {
    mocks.hubRuntimeRelocationInstanceId.mockReturnValueOnce(instanceId);
    mocks.preflightHubRuntimeRelocation.mockReturnValueOnce({
      runtimeId,
    ...
    expect(mocks.preflightHubRuntimeRelocation).toHaveBeenCalledWith(payload);
  });
  ...
  it.each([
    ...
    if (code === "relocation_instance_mismatch") {
      mocks.preflightHubRuntimeRelocation.mockImplementationOnce(() => {
        throw new RuntimeRelocationPreflightError(
          "Target Hub instance does not match the source instance.",
          code,
        );
      });
    }
    ...
    expect(mocks.preflightHubRuntimeRelocation).toHaveBeenCalledTimes(
