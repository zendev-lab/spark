...
describe("workspace control display", () => {
  it("renders borrowed and online executor state for Hub", () => {
    expect(
      ...
      controlLabel:
        "Another interactive session is using this workspace. Close or release it before running actions from Hub.",
    });
  });
  ...
  it("renders workspace control labels from the Chinese dictionary", () => {
    ...
    ).toBe("Spark daemon 当前离线。重新连接后，才可从 Hub 执行操作。");
  });
});
