<script lang="ts">
  ...
  type Messages = HubMessages["sessions"];
  ...
  let startSlashActionBar = $derived.by(() => {
    ...
    return view ? localizeHubSlashActionBar(view, copy.slashActions) : undefined;
  });
  ...
  let startSlashSuggestions = $derived.by(() =>
    composer.startSlashDismissedInput === composer.startMessage
      ? []
      : hubSlashSuggestionsForInput(composer.startMessage, copy.slashActions),
  );
  let sessionSlashSuggestions = $derived.by(() =>
    composer.sessionSlashDismissedInput === composer.message
      ? []
      : hubSlashSuggestionsForInput(composer.message, copy.slashActions),
  );
