<script lang="ts">
  ...
  async function startNotifications() {
    ...
    if (!config.configured || !config.publicKey) {
      throw new Error("Web Push is not configured on this Hub server.");
    }
  ...
  function connectEvents() {
    stopEventSource();
    const cursorKey = "spark-hub:notifications:events-cursor";
    ...
    eventSource.addEventListener("spark-hub.event", (message) => {
      ...
      const notification = notificationFromHubEvent(event);
  ...
  function showNotification(notification: HubNotificationPayload) {
