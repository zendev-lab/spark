...
describe("pending ask helpers", () => {
  it("encodes option and custom replies in the structured ask answer shape", () => {
    ...
    expect(humanSingleAnswerWithCustomFallback(single, hubCustomAnswerValue, "  custom  ")).toEqual(
      { values: [], customText: "custom" },
    );
    expect(
      humanMultiAnswerWithCustomFallback(multi, ["web", hubCustomAnswerValue], "  native app  "),
    ).toEqual({ values: ["web"], labels: ["Web"], customText: "native app" });
    expect(humanMultiAnswerWithCustomFallback(multi, [hubCustomAnswerValue], "  ")).toEqual({
      values: [],
    });
    expect(
      humanSingleAnswerWithCustomFallback(
