...
describe("login route boundary", () => {
  it("preserves separate root and workspace server auth policies", () => {
    ...
    expect(rootServer).toContain("exchangeHubAccessToken");
    expect(rootServer).toContain("getCurrentHubSession");
    ...
    expect(workspaceServer).not.toContain("exchangeHubAccessToken");
  });
