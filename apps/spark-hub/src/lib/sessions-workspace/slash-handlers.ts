...
export type SlashHandlerDeps = {
  ...
  getStartSlashSuggestions: () => readonly HubSlashCommandSuggestion[];
  getSessionSlashSuggestions: () => readonly HubSlashCommandSuggestion[];
  ...
  submitDirectiveSelection: (directive: "plan" | "execute" | "fleet") => Promise<void>;
...
function directiveFromAction(action: SparkActionView): "plan" | "execute" | "fleet" | null {
  const candidate = action.payload.directive;
...
export function createSlashHandlers(deps: SlashHandlerDeps) {
  ...
  async function handleSlashAction(action: SparkActionView, surface: ComposerSurface) {
    ...
    if (action.intent === "directive.run") {
      const directive = directiveFromAction(action);
      if (!directive) return;
      clearSlashInput(surface);
      await deps.submitDirectiveSelection(directive);
      return;
    }
    ...
    if (action.intent === "help.commands") {
      clearSlashInput(surface);
      window.dispatchEvent(new CustomEvent(hubOpenSearchEvent));
      return;
    }
