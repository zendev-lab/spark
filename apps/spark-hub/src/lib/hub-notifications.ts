import type { SerializedEvent } from "@zendev-lab/spark-hub-coordination/events";

export const notificationPreferenceStorageKey = "spark-hub:notifications:enabled";
...
export function notificationFromHubEvent(
  ...
  if (event.kind === "invocation.updated") {
    ...
    return {
      ...
      body: ok
        ? "A long-running Spark task completed. Open Hub to review the result."
        : "A long-running Spark task stopped or failed. Open Hub to review the status.",
...
export function sanitizeNotificationPayload(value: unknown): HubNotificationPayload | null {
  ...
  const body = safeText(record.body, "Open Hub to review.");
