...
describe("channel settings page contract", () => {
  ...
  it("saves account configuration without managed-session mutations", () => {
    ...
    expect(calls.has("saveChannelsConfigForHub")).toBe(true);
    expect(calls.has("requireSecretRequestContext")).toBe(true);
    expect(calls.has("createManagedSessionForHub")).toBe(false);
    expect(calls.has("bindManagedSessionForHub")).toBe(false);
    expect(calls.has("archiveManagedSessionForHub")).toBe(false);
  ...
  it("renders QR locally while keeping returned QQ secrets daemon-owned", () => {
    ...
    expect(server).toContain("createHubRuntimeModelChannelClient().startQqbotQrAuth");
