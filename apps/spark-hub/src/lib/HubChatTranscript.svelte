<script lang="ts">
  ...
  import {
    ...
    latestActivity,
    type HubChatCommand,
    type HubChatInvocation,
    type HubChatLogChunk,
    type HubChatTranscriptTurn,
  } from "$lib/hub-chat-transcript-view";

  type Command = HubChatCommand & {
  ...
  type Invocation = HubChatInvocation;
  type LogChunk = HubChatLogChunk;
  ...
  let transcriptTurns = $derived(
    buildHubChatTranscriptTurns(commands, invocations, logChunks, {
