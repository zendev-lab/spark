...
import { handleSparkHubCliCommand } from "./coordination.ts";
import {
...
test("daemon/Hub/TUI golden path stays contract-focused and credential-free", async () => {
  ...
  try {
    ...
    const hub = await handleSparkHubCliCommand(
    ...
    assert.equal(hub.action, "status");
    const hubContract = extractHubStatusContract({
      action: "status",
      result: hub.result,
    });
    assert.deepEqual(hubContract.diagnostics, []);
    assert.equal(hub.result.plane, "hub");
    assert.equal(hub.result.resource, "status");
    assert.equal(hub.result.currentProjectRef, project.ref);
    assert.equal(hub.result.readyTasks[0]?.taskRef, ready.ref);
    assert.deepEqual(hub.result.scope, {
