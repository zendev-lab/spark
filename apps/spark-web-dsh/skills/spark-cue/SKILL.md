...
For `script_run`, Python script files execute through `uv run --script <path>`. For `script_eval`, inline Python is piped to `uv run --script -`. `venv` is valid only with `language="python"` and selects `<venv>/bin/python` via `uv run --python <venv>/bin/python` (combined with `--script <path>` for files or `--script -` for inline Python). `script_eval` previews a fixed, bounded leading snippet of inline code in the rendered tool call without exposing display-only parameters in the callable schema.
...
For resource-gated jobs, pass `needs` as an object whose keys omit the `need.` prefix. Examples: `needs={ gpu: 1, gpu_mem: "24GiB" }` or `needs={ license: 1 }`. **Do not** put `:run(need.gpu=1)` inside `command`; `spark-cue` already wraps `command` in `:run(...)` and encodes `needs` as mode params.
