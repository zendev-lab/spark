...
await build({
  entryPoints: ["src/private-inner-auth.ts"],
  bundle: true,
  format: "esm",
  platform: "node",
  target: "node22",
  outfile: "lib/spark-private-inner-auth.mjs",
  external: ["@deepseek-ai/*"],
  minify: true,
  logLevel: "info",
});
...
await build({
  entryPoints: ["../../packages/dsh-tool-fusion/src/extension.ts"],
  bundle: true,
  format: "esm",
  platform: "node",
  target: "node22",
  outfile: "lib/dsh-tool-fusion.mjs",
  external: ["@deepseek-ai/*"],
  minify: true,
  logLevel: "info",
});
