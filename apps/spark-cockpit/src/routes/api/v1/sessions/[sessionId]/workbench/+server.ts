...
import {
  controlReproWorkbenchForCockpit,
  loadProjectedReproWorkbench,
  ReproWorkbenchControlError,
} from "$lib/server/repro-workbench";
import { CockpitRuntimeSessionUnavailableError } from "$lib/server/cockpit-runtime-session-client";
...
export const POST: RequestHandler = async ({ params, request }) => {
  ...
  try {
    const result = await controlReproWorkbenchForCockpit({
      db: getDatabase(),
