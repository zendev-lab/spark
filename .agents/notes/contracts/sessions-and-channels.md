...
`role` owns static definitions and model settings only. Public Role execution is a three-stage flow: create or select a static Role, call `session spawn` or `session fork` with its exact `RoleRef`, then call `session send(kind=request)` to admit work. Role creation never creates a Session, and Session creation never creates mail or an Invocation. Internal Task, Workflow, Driver, Channel, Scheduler, and `skill_agent` owners may continue to use the daemon's internal Role execution engine.
...
Public `session spawn` and `session fork` create ordinary supervisor-owned scoped child Sessions. Both require an exact existing `RoleRef`; tool calls use the current Session as supervisor and CLI calls require `--supervisor`. `spawn` starts with an empty transcript. `fork` can copy only its supervisor's stable transcript prefix, never an arbitrary source Session. Neither accepts an instruction, sends mail, wakes the child, or creates an Invocation. Closed, archived, channel-bound, or ephemeral supervisors fail before registry mutation.

Each fork child has an independent registry record and canonical JSONL. The daemon copies through the last normally completed assistant entry, excluding `tool_use`/`tooluse`, aborted, error, and in-flight tails; if no stable assistant exists the child is empty. Copied entry IDs and `parentId` links retain their transcript-local values. Parent and child then append, compact, and close independently, so no writable tail is shared.

The daemon fingerprints the parent transcript before and after reading, retries once if it changes, and then returns typed `session_transcript_changed` rather than committing a torn snapshot. It writes the child JSONL through a temporary file and atomic rename before registry creation; synchronous registry failure removes that exact file. Side Thread contextual seeding uses the same stable-prefix selector while retaining its hidden seed boundary and handoff semantics.

Selecting `+ New session` in the TUI allocates a provisional ID only. The daemon persists the Session on the first operation that needs durable state, uses the default `none` Role binding, and parents it to the Workspace Administrator. Hub and ACP use the same internal provisioning contract, so opening and immediately closing a blank conversation cannot grow the registry or create a parallel root.
