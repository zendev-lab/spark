...
A provider completion without displayable text or a tool call is a typed transient failure. The accepted invocation may use the bounded, checkpoint-aware provider-continuation policy: requests without completed tool receipts may be resubmitted, while a completed receipt is persisted before continuation so the tool side effect is not repeated. If that continuation budget is exhausted, the invocation remains a retryable terminal failure and only explicit `invocation.retry` creates a new linked invocation.
...
Restart is asynchronous and drain-first. An IPC-ready external watchdog and atomically persisted restart fence are armed before the old process closes admission. The old process then stops claiming queued work, rejects new direct runtime execution with `DAEMON_DRAINING`, and flushes already-received channel admissions. An eligible persistent local/web turn that finishes a model response with pending tool calls atomically changes its running invocation back to `queued`, recording the persisted-session head, transient host prompt items, and exact pending calls before any tool dispatch. The predecessor treats that control-flow yield as non-terminal and does not emit `agent_end`; the successor validates the unchanged session head, restores the prompt, dispatches the pending calls once, and continues the model loop. Work already executing a tool, channel/Loop turns, reset/hidden sessions, and unsupported execution paths still drain to actual settlement. The detached successor helper retries startup with capped backoff and no attempt limit; only a durable cancel, completed, or superseded fence stops it. The successor must report the exact fenced restart ID, instance ID, generation, PID, and protocol version, and publishes completion only after local admission and serving loops are active. Supervised external channel connections are not readiness gates. The CLI returns after acceptance by default; an external caller may use `--wait`. Replacement preserves single ownership and therefore has a short socket handoff window rather than running two active daemon generations.
...
SQLite remains the delivery authority. A healthy connection may use persisted live events as an in-memory accelerator, but it registers that sink before `runtime.hello` and performs a bounded paged catch-up after `server.hello_ack`. Sequence gaps, buffer overflow, workspace-binding changes, and periodic reconciliation return to the durable cursor. A page preserves sequence order within each invocation even when event timestamps regress. Acknowledging one item performs only the durable cursor update and drains the already-loaded page; page reads and projection-drop runs execute in bounded macrotasks so Hub delivery cannot monopolize daemon admission. Closing a connection discards only its memory buffer and never advances an unacknowledged cursor.
