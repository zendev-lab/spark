...
| `spark-files` | retain as a Spark domain owner | Atomic/versioned workspace IO and Artifact-root semantics exceed a tool adapter. Reuse official `dsh-fs`, `dsh-tool-fs-search`, and `read_image`; adapt Spark's versioned text tools to `ctx.fs` because rc.8 statically advertises escalation fields that can be impossible for the live session. Do not rename the owner. |
| `spark-fusion` | renamed to `dsh-tool-fusion` in this stack | It is a stateless, model-facing tool family with no official DSH equivalent or durable state owner. Its only ABI is the Cordis plugin root; the SparkHostAPI bridge has been removed. |
