# Hub instance relocation

This is a feature-only procedure for moving one logical Hub instance to a new HTTPS origin. It is not an independent Hub registration and it is not an origin-lease transfer. The daemon owns the workspace directory; Hub only holds an exclusive **active origin lease** while sessions **occupy** that directory. Relocation does not authorize an operator to upload, restore, deploy, or switch any existing deployment.
...
- any secret appears in CLI JSON, Hub persistence/cache/log, generic outbox, or artifacts.
...
```sh
export SOURCE_URL=https://source-hub.example.com/
export TARGET_URL=https://target-hub.example.com/
export SOURCE_DB=/srv/spark/source/hub.sqlite
export TARGET_DB=/srv/spark/target/hub.sqlite
...
| Origin lease | Which Hub origin currently projects the directory (`workspace_leases` + uplink `prefer`/`park`) |
| Session occupancy | Interactive TUI / Hub workbench sessions that gate silent transfer |
...
```sh
spark hub instance status --database "$SOURCE_DB" --json
...
```sh
spark hub instance backup --database "$SOURCE_DB" --snapshot "$SNAPSHOT" --json
spark hub instance inspect --snapshot "$SNAPSHOT" --json
```
...
Stop the target Hub process **or** wait until its database lock is free. Hub opens SQLite on demand with a 30s idle release after the last consumer (HTTP request, SSE stream, runtime WebSocket, or web-push tick) unpins. HTTP may keep listening while the lock is idle; restore must still refuse while any consumer holds the lock (active browsers or runtime WSS). The service command is deployment-specific; for a user service:

```sh
systemctl --user stop spark-hub
spark hub instance backup --database "$TARGET_DB" --snapshot "$TARGET_PREBACKUP" --json
spark hub instance inspect --snapshot "$TARGET_PREBACKUP" --json
spark hub instance restore --snapshot "$SNAPSHOT" --database "$TARGET_DB" --rollback-root "$TARGET_ROLLBACK_ROOT" --yes --json
spark hub instance status --database "$TARGET_DB" --json
```
...
export PORT=5173
export SPARK_HUB_PUBLIC_URL="$TARGET_URL"
export SPARK_HUB_TRUST_PROXY=loopback
spark hub
```
...
rejection-only. A legacy endpoint such as `http://legacy-hub.example.com:8080/` is not eligible
...
Do not restore the target's old database while the daemon points at the target. First reverse-relocate the live logical Hub state so the old source can authenticate the daemon's current rotated credential:
...
spark hub instance backup --database "$TARGET_DB" --snapshot "$RETURN_SNAPSHOT" --json
spark hub instance inspect --snapshot "$RETURN_SNAPSHOT" --json
systemctl --user stop spark-hub-source
spark hub instance backup --database "$SOURCE_DB" --snapshot "$SOURCE_PREBACKUP" --json
spark hub instance restore --snapshot "$RETURN_SNAPSHOT" --database "$SOURCE_DB" --rollback-root /srv/spark/backups/source-automatic --yes --json
systemctl --user start spark-hub-source
...
```sh
systemctl --user stop spark-hub
spark hub instance restore --snapshot "$TARGET_PREBACKUP" --database "$TARGET_DB" --rollback-root "$TARGET_ROLLBACK_ROOT" --yes --json
spark hub instance status --database "$TARGET_DB" --json
```
