...
export interface ReflectionCommandApi {
  registerCommand(
    ...
    config: {
      description: string;
      handler: (args: string, ctx: ReflectionCommandContext) => void | Promise<void>;
    },
  ): void;
...
export function registerSparkReflectionCommands(pi: ReflectionCommandApi): void {
...
export async function runReflectionOnce(
  ...
  try {
    ...
    const scan = await scanSparkSessionHistory({ sessionRoot, cursor });
...
export function startReflectionScheduler(
  pi: ReflectionCommandApi,
  ctx: ReflectionCommandContext,
...
export function stopReflectionScheduler(ctx: Pick<ReflectionCommandContext, "cwd">): boolean {
  const key = reflectionSessionKey(ctx);
...
function sendReflectionReport(pi: ReflectionCommandApi, summary: ReflectionRunSummary): void {
  pi.sendMessage({
...
function sendReflectionNotice(pi: ReflectionCommandApi, content: string): void {
