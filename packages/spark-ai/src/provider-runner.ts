...
function retryProviderStreamBeforeOutput(
  ...
  createStream: () => AsyncIterable<AssistantMessageEvent> & {
    result(): Promise<AssistantMessage>;
  },
  providerName: string,
  ...
  const retryingStream: AsyncIterable<AssistantMessageEvent> & {
    result(): Promise<AssistantMessage>;
  } = {
    async *[Symbol.asyncIterator]() {
      if (consumed) {
        if (final) return;
        throw new Error(`Provider "${providerName}" stream is already being consumed`);
      }
      consumed = true;
      let retries = 0;
      ...
      while (true) {
        ...
        for await (const event of stream) {
          ...
          if (event.type === "error") {
            const retriable =
              ...
              classifyProviderFailure(event.error).failureClass === "transient";
            if (retriable) {
              retry = true;
              break;
            }
        ...
        if (!retry) {
          throw new Error(`Provider "${providerName}" stream ended without a terminal event`);
        }
        retries += 1;
...
function providerStreamRetryDelayMs(attempt: number, options: StreamOptions | undefined): number {
