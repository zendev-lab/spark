<script lang="ts">
  type Props = {
    ...
    name?: string;
    /** Surface-owned media URL. Defaults to the Hub media route. */
    mediaHref?: string;
  };

  let { sessionId, messageId, contentIndex, mediaType, name, mediaHref }: Props = $props();
  let src = $derived(
    mediaHref ??
...
<figure class="message-image">
  <a href={src} target="_blank" rel="noreferrer" aria-label={name ?? "Image"}>
    <img src={src} alt={name ?? ""} loading="lazy" decoding="async" data-media-type={mediaType} />
  </a>
