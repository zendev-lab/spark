...
test("normalizeProjectKindId returns generic for empty/null/undefined", () => {
...
test("normalizeProjectKindId passes through non-empty strings", () => {
  assert.equal(normalizeProjectKindId("reproduction"), "reproduction");
  assert.equal(normalizeProjectKindId("custom"), "custom");
});

test("renderSparkProjectKindDisplay returns no-op display with no panels", () => {
...
test("renderSparkProjectKindDisplay returns generic for undefined kind", () => {
  const display = renderSparkProjectKindDisplay({ kind: undefined });
  assert.equal(display.kind, "generic");
  assert.deepEqual(display.panels, []);
});

test("sparkProjectKindRoleForPhase always returns undefined (deprecated)", () => {
...
test("evaluateSparkProjectKindCompletionGate always returns ok=true (deprecated)", () => {
...
test("evaluateSparkProjectKindCompletionGate ok even with empty kindState", () => {
