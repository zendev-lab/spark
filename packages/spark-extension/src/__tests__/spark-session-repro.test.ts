...
describe("SparkSessionRepro evidence-backed state machine", () => {
  ...
  it("normalizes legacy plan-array Formalize order into stage order without promoting proof", async () => {
    ...
    try {
      ...
      const path = sessionReproStorePath(dir);
      await mkdir(dirname(path), { recursive: true });
      await writeFile(path, `${JSON.stringify({ version: 9, repro })}\n`, "utf8");
      ...
      await writeFile(path, `${JSON.stringify({ version: 9, repro: legacy })}\n`, "utf8");
  ...
  it("preserves an invalid v9 snapshot instead of overwriting it as empty", async () => {
    ...
    try {
      ...
      const path = sessionReproStorePath(dir);
      await mkdir(dirname(path), { recursive: true });
      const serialized = `${JSON.stringify({ version: 9, repro: invalid })}\n`;
