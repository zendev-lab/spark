...
import {
  ...
  renderSparkWorkflowLoopPrompt,
  renderSparkWorkflowLoopVisibleMessage,
} from "./spark-phase-prompts.ts";
...
export async function enterSparkUltracodeWorkflow(
  ...
  ctx.ui?.notify?.("Spark ultracode Workflow selected.", "info");
...
export async function enterSparkWorkflow(
  ...
  ctx.ui?.notify?.("Spark Workflow selected.", "info");
  const savedWorkflows = await discoverSparkSavedWorkflows(sparkStateCwd(ctx.cwd, ctx));
  await dispatchSparkAgentInstruction(
    piApi,
    deps,
    ctx,
    renderSparkWorkflowLoopPrompt(focus, savedWorkflows, workflowSelector),
    renderSparkWorkflowLoopVisibleMessage(focus, workflowSelector),
  );
}
