...
export async function collectSparkStateDiagnostics(
  ...
  return {
    ...
    orphanBlobs: boundedDiagnostics(evidenceInventory.orphanBlobs),
    ...
    for (const file of files) {
      ...
      if (!kind || !subjectRef) {
        findings.push({
          ...
          repair:
            "Recreate the review record from review evidence or move it to import quarantine.",
        });
        continue;
      }
...
async function collectSparkEvidenceDiagnostics(
  ...
  for (const file of metadataFiles) {
    ...
    const resolvedBlobPath = blobPath ? resolveEvidenceBlobPath(evidenceRoot, blobPath) : undefined;
