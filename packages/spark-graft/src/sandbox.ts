...
function defaultSandboxWorkspace(cwd: string, name?: string): string {
  const safeName = (name ?? cwd)
    ...
    .replace(/^-+|-+$/g, "")
    .slice(0, 80);
  return resolve(tmpdir(), "spark-graft-sandbox", safeName || "workspace");
}

const SANDBOX_PROFILE_RELOAD_GUIDANCE =
  "Sandbox profile caveat: file-tool names read/write/edit/grep/find/ls remain sandbox overrides until Pi reloads without @zendev-lab/spark-graft/sandbox; graft_sandbox_exit only clears sandbox state.";
...
export function registerSparkGraftSandboxExtension(pi: SparkGraftHostApi): void {
        ...
        content: [
          {
            type: "text",
            text: previous
              ? `Exited Graft sandbox and cleared sandbox state. Retained refs: scratch=${previous.lastScratch ?? "none"}, candidate=${previous.lastCandidate ?? "none"}, patch=${previous.lastPatch ?? "none"}. File-tool names read/write/edit/grep/find/ls remain sandbox overrides in this loaded profile; reload without @zendev-lab/spark-graft/sandbox, and do not use --no-builtin-tools if ordinary Pi built-ins should be available.`
  ...
  pi.registerTool({
    ...
    async execute(
      ...
      candidateArgv.push(
        "--producer",
        optionalStringParam(params, "producer") ?? "@zendev-lab/spark-graft-sandbox",
      );
