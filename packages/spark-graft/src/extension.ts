...
function stateSummary(state: ActiveGraftScratchState | undefined): string {
  if (!state) {
    return "No spark-graft convenience state. Start scratch tools with base, or continue a returned scratch with from.";
  }
  return [
...
function assertCliExecAllowed(argv: string[]): void {
  ...
  throw new Error(
    `graft_cli_exec does not allow graft ${command}. Use typed spark-graft tools for scratch/patch lifecycle operations, or graft_help for supported low-frequency CLI workflows.`,
  );
}
...
function convertHashlineEdit(edit: unknown, index: number): JsonRecord {
  ...
  if ("oldText" in edit || "newText" in edit || "old_text" in edit || "new_text" in edit) {
    throw new Error(
      `edits[${index}] uses exact-text replacement fields; spark-graft edit is strict hashline-only.`,
    );
  }
  const op = edit.op;
...
function toolCwd(
  ...
  if (!cwd) throw new Error("spark-graft tools require a cwd context or restored session state.");
  return cwd;
}
  ...
  defineGraftTool({
    ...
    async execute(
      ...
      if (
        ...
        throw new Error(
          "spark-graft graft_edit is strict hashline-only; call graft_read first and use edits[].op/pos/lines.",
        );
      }
      const cwd = toolCwd(ctx, activeState, lastCwd);
      const previousState = stateForCwd(activeState, cwd);
  ...
  defineGraftTool({
    ...
    parameters: Type.Object({
      ...
      producer: Type.Optional(
        Type.String({ description: "Candidate provenance producer. Defaults to spark-graft." }),
      ),
