...
Goal continuation code now lives under `packages/spark-driver` alongside the non-completing loop substrate. `@zendev-lab/spark-driver` is the single package boundary for reusable loop and goal primitives, while Spark-specific command/tool/reviewer policy remains in `apps/spark-daemon/src/product`.
