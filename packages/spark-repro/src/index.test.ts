...
describe("spark-repro", () => {
  ...
  it("holds a completed v6 snapshot for explicit v7 revalidation", () => {
    ...
    expect(migrated.status).toBe("needs_revalidation");
    expect(migrated.completedAt).toBeUndefined();
    expect(migrated.stopGuard.decision).toBe("revalidate");
    const settled = settleReproTick(migrated);
    expect(settled.decision).toBe("revalidate");
    expect(settled.scheduleDelayMs).toBeUndefined();
    expect(settled.repro.status).toBe("needs_revalidation");
    const revalidating = beginReproRevalidation(migrated);
    expect(revalidating.status).toBe("active");
    expect(revalidating.stopGuard.decision).toBe("continue");
