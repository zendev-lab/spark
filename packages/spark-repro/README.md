...
proof into Normative retirement. A legacy `complete` record becomes
`needs_revalidation`; loading or settling it cannot resume execution. Only an
explicit `beginReproRevalidation` transition reopens it as active. A later plan
revision preserves observation and unresolved identities, but resets revision-bound
candidates and retirement rather than inferring them from legacy `done` status.
