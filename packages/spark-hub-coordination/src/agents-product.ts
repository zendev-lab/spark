...
export { sparkAgentsHubSource as agentsHubSource };
...
function isAgentsHubCommand(command: { payloadJson: string }) {
  try {
    ...
    return Boolean(
      ...
      (payload as { source?: unknown }).source === sparkAgentsHubSource,
    );
  } catch {
    return false;
  }
...
function loadArtifactFallbackLogChunks(
  ...
  for (const row of rows) {
    ...
    const text = assistantTextFromProjectedArtifact(db, row);
    if (!text) continue;
    fallbackLogs.push({
...
function assistantTextFromProjectedArtifact(
  ...
  const preview = readArtifactPreviewContent(db, row.id);
  if (preview.cache.previewStatus !== "ready" || !preview.body) return null;
  const body = preview.body.toString("utf8");
  return assistantTextFromRoleRunBody(
    preview.cache.mime === "application/json" ? parseJson(body) : body,
  );
}
