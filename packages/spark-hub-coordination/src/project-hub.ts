...
function parseJsonObject(value: string | null | undefined): Record<string, unknown> {
  if (!value) return {};
  const parsed = parseJson(value, "project hub object");
...
function parseJsonArray(value: string | null): string[] {
  ...
  const parsed = parseJson(value, "project hub array");
  return Array.isArray(parsed)
    ? parsed.filter((item): item is string => typeof item === "string")
    : [];
}

function normalizeProjectHubKindDisplay(value: unknown): ProjectHubKindDisplay | null {
  ...
  const panels = Array.isArray(value.panels)
    ? value.panels.flatMap((panel): ProjectHubKindPanel[] => {
...
function isProjectKindRender(value: unknown): value is ProjectHubKindPanel["render"] {
...
export interface ProjectHubTaskLink {
  runtimeTaskId: string;
  title: string;
  kind: string;
}

export interface ProjectHubInvocationLink {
  id: string;
...
export interface ProjectHubKindPanel {
  label: string;
  render: "text" | "progress" | "counts" | "list";
  text: string;
}

export interface ProjectHubKindDisplay {
  kind: string;
  title: string;
  badge?: string;
  panels: ProjectHubKindPanel[];
}
...
export interface ProjectHubTask {
  ...
  runIds: string[];
  blockers: ProjectHubTaskLink[];
  dependents: ProjectHubTaskLink[];
  invocationLinks: ProjectHubInvocationLink[];
  readyFrontier: boolean;
}

export interface ProjectHubLease {
...
export interface ProjectHubCommand {
  id: string;
  kind: string;
  title: string | null;
  payloadJson: string;
...
export interface ProjectHubLogChunk {
  id: string;
  runtimeInvocationId: string;
  agentName: string | null;
  stream: string;
...
export function loadProjectHub(db: DatabaseSync, projectId: string) {
  ...
  const lease =
    (db
      ...
      .get(project.workspaceId) as ProjectHubLease | undefined) ?? null;
  ...
  const projectKind = normalizeProjectHubKindDisplay(
  ...
  const tasks: ProjectHubTask[] = taskRows.map((task) => {
  ...
  const commands = db
    ...
    .all(project.id) as unknown as ProjectHubCommand[];
  ...
  const logChunks = (
    db
      ...
      .all(project.id) as unknown as ProjectHubLogChunk[]
  ).reverse();
...
export type ProjectHub = NonNullable<ReturnType<typeof loadProjectHub>>;
