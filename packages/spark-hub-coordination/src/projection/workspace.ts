...
/**
 * End only the Hub origin lease projection. The daemon-owned directory and its
 * runtime binding row remain intact so a reconnect can observe the unbound state
 * and the directory can later be attached to another Hub workspace.
 */
...
/**
 * Remove a workspace from the Hub directory without deleting daemon-owned
 * local directories or sessions. Archives the workspace row (freeing its slug)
 * and ends any active Hub origin lease.
 */
...
function ensureActiveWorkspaceLease(
  ...
  if (activeWorkspaceForBinding && activeWorkspaceForBinding.workspaceId !== workspaceId) {
    throw new Error(
      `Runtime workspace binding already belongs to another Hub workspace: ${runtimeWorkspaceBindingId}`,
    );
  }
  const active = db
    .prepare(
