...
export function createProject(db: DatabaseSync, input: CreateProjectInput) {
  return withTransaction(db, () => {
    ...
    db.prepare(
      ...
      toJson({ sourceOfTruth: "spark-hub-routing" }),
      timestamp,
      timestamp,
    );
...
export type HubWorkspaceDaemonConnectionStatus = "connected" | "disconnected";

export interface HubWorkspaceControlProjection {
  workspaceId: string;
  runtimeWorkspaceBindingId: string | null;
  connection: {
    status: HubWorkspaceDaemonConnectionStatus;
...
/**
 * Queue a server command in the Hub projection outbox for daemon delivery.
 *
...
export function queueCommandForWorkspaceLease(db: DatabaseSync, input: QueueCommandInput) {
  return withTransaction(db, () => {
    assertHubMayWriteScope("commands");
...
export function loadWorkspaceServerControl(
  ...
  const snapshot = lease
    ...
    : null;
  const connectionStatus: HubWorkspaceDaemonConnectionStatus =
...
/** Blocks Hub mutations when a non-hub interactive session holds occupancy. */
function hasForeignInteractiveOccupancy(
  borrowed: WorkspaceBorrowedState,
  workspaceClients: unknown,
): boolean {
  if (borrowed.sessions.some((session) => session.surface !== "hub")) return true;
  ...
  return interactive.some((client) => client.surface !== "hub");
}
