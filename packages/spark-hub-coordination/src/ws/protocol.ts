...
export function handleMvpRuntimeMessage(
...
export function sendReconcileRequest(
  ws: WebSocket | RuntimeWebSocketConnection,
...
export function upsertWorkspaceBindings(
  db: DatabaseSync,
  runtimeId: string,
...
export function listWorkspaceBindingAssignments(
  db: DatabaseSync,
  runtimeId: string,
  bindingIds: string[],
) {
...
function requireRoutedContext(
  ...
  if (
    ...
    sendError(
      ...
      "Runtime message did not match the active Hub origin lease.",
    );
    return null;
  }
...
export function flushPendingRuntimeDeliveries(
  ws: WebSocket | RuntimeWebSocketConnection,
  context: RuntimeWebSocketContext,
): void {
...
export function runtimeTokenScopes(scopesJson: string): string[] {
  try {
