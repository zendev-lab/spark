...
describe("migrations", () => {
  it("applies the MVP projection schema to an empty database", () => {
    ...
    const delegationColumns = db
      .prepare("PRAGMA table_info(workspace_delegations)")
      .all() as Array<{ name: string }>;
    expect(delegationColumns.map((column) => column.name)).toContain("target_session_generation");
    db.close();
  });
  ...
  it("records fresh migrations clean and adopts an existing schema honestly", () => {
    ...
    expect(
      existing
        ...
        .all(),
    ).toEqual([{ state: "legacy-unverified", count: 24 }]);
    expect(
      existing
        .prepare(
