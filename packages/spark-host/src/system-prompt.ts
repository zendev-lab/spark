...
export const SPARK_OPERATING_POLICY_PROMPT = [
  ...
  "Proceed without another confirmation for in-scope reads, local edits, non-destructive validation, and reversible high-confidence work already authorized by the request. On a daemon-owned Goal, Loop, or Repro continuation turn, also proceed with external operations whose tool owner explicitly marks them manual_only when they remain bounded, low-risk, reversible, and inside the confirmed objective and target. Manual turns require human approval for manual_only operations; destructive, irreversible, security-sensitive, costly, high-impact, materially scope-expanding, unstructured command or script execution, release, deployment, merge, and other required operations always require human approval. Automated review and model confidence are not user authorization.",
