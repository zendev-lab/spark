...
test("spark-loop package owns goal primitives and stays isolated from workflow packages", async () => {
  ...
  const sourceFiles = await listTypeScriptFiles(new URL(".", import.meta.url));
...
async function listTypeScriptFiles(dir: URL): Promise<URL[]> {
  const entries = await readdir(dir, { withFileTypes: true });
  const files: URL[] = [];
  for (const entry of entries) {
    const path = new URL(entry.name + (entry.isDirectory() ? "/" : ""), dir);
    if (entry.isDirectory()) files.push(...(await listTypeScriptFiles(path)));
    else if (entry.isFile() && path.pathname.endsWith(".ts")) files.push(path);
  }
  return files;
}
