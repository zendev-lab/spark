...
surface. This is a `spark-tool-<family>` adapter, not the `spark web` app.
...
Persisted `responseId` values keep the `spark-web:` prefix. Persisted configs
still listing `@zendev-lab/spark-web/extension` rewrite to
