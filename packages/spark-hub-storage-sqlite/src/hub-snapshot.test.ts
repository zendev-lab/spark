...
function createRoot(): string {
  const root = mkdtempSync(join(tmpdir(), "spark-hub-snapshot-"));
  roots.push(root);
  return root;
}
...
describe("Hub instance snapshots", () => {
  it("preserves durable identity and history while invalidating origin-bound transient state", async () => {
    ...
    const manifest = await createHubSnapshot({
      sourceDb: source,
    ...
    expect(inspectHubSnapshot(snapshotPath)).toMatchObject({
      integrityCheck: "ok",
    ...
    target.close();
    const result = await restoreHubSnapshot({
      snapshotPath,
    ...
    expect(inspectHubSnapshot(result.rollbackSnapshotPath!).manifest.instanceId).toBe(
      targetInstanceId,
    );

    const restored = new DatabaseSync(targetPath, { readOnly: true });
    try {
      expect(setting(restored, "spark_hub:instance_id")).toBe(sourceInstanceId);
      expect(setting(restored, "durable.setting")).toEqual({ retained: true });
      expect(setting(restored, "spark_hub:web_push_subscription")).toBeNull();
    ...
    const rollback = new DatabaseSync(join(result.rollbackSnapshotPath!, "hub.sqlite"), {
      readOnly: true,
    });
    try {
  ...
  it("rejects a corrupted snapshot before changing the target", async () => {
    ...
    appendFileSync(join(snapshotPath, "hub.sqlite"), "corruption", "utf8");
  ...
  it("rejects foreign-key violations before changing the target", async () => {
    const { root, snapshotPath, targetPath } = await snapshotAndTarget();
    const snapshotDbPath = join(snapshotPath, "hub.sqlite");
  ...
  it("refuses restore while another process holds the Hub database lock", async () => {
    const { root, snapshotPath, targetPath } = await snapshotAndTarget();
    const lockPath = hubDatabaseLockPath(targetPath);
    const script = `
    ...
    try {
      ...
      ).rejects.toBeInstanceOf(HubDatabaseLockedError);
      expect(sha256(targetPath)).toBe(before);
    } finally {
      child.kill("SIGTERM");
  ...
  it("restores the original target when atomic replacement fails after moving it", async () => {
    ...
    try {
      expect(setting(target, "spark_hub:instance_id")).toBe(targetInstanceId);
  ...
  it("repairs an invalid stored instance id atomically", () => {
    ...
    const repaired = ensureHubInstanceId(db, { instanceId: sourceInstanceId, now });
    expect(repaired).toBe(sourceInstanceId);
    expect(setting(db, "spark_hub:instance_id")).toBe(sourceInstanceId);
    db.close();
  });
...
function seededSourceDatabase(path: string): ReturnType<typeof openDatabase> {
  ...
  migrate(db);
  ensureHubInstanceId(db, { instanceId: sourceInstanceId, now });
  db.prepare("INSERT INTO app_settings (key, value_json, updated_at) VALUES (?, ?, ?)").run(
    "durable.setting",
  ...
  db.prepare("INSERT INTO app_settings (key, value_json, updated_at) VALUES (?, ?, ?)").run(
    "spark_hub:web_push_subscription",
...
function seededTargetDatabase(path: string): ReturnType<typeof openDatabase> {
  const db = openDatabase({ path });
  migrate(db);
  ensureHubInstanceId(db, { instanceId: targetInstanceId, now });
  db.prepare(
    `INSERT INTO users (id, display_name, role, status, created_at, updated_at)
