...
export interface HubSnapshotInspection {
  snapshotPath: string;
  databasePath: string;
  manifest: HubSnapshotManifest;
  integrityCheck: "ok";
  foreignKeyViolations: 0;
}

export interface HubDatabaseLockRecord {
  pid: number;
  acquiredAt: string;
  databasePath: string;
  ownerToken: string;
...
export interface HubDatabaseLockHandle {
  path: string;
  record: HubDatabaseLockRecord;
  release(): void;
}
...
export class HubDatabaseLockedError extends Error {
  readonly lockPath: string;
  readonly record: Partial<HubDatabaseLockRecord> | null;

  constructor(lockPath: string, record: Partial<HubDatabaseLockRecord> | null) {
    const owner = record?.pid ? ` by process ${record.pid}` : "";
    super(`Spark Hub database is locked${owner}: ${lockPath}`);
    this.lockPath = lockPath;
    this.record = record;
    this.name = "HubDatabaseLockedError";
  }
}

export class HubSnapshotValidationError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "HubSnapshotValidationError";
  }
}
...
export function hubDatabaseLockPath(databasePath: string): string {
  return `${resolve(databasePath)}.lock`;
}

export function acquireHubDatabaseLock(databasePath: string): HubDatabaseLockHandle {
  const resolvedDatabasePath = resolve(databasePath);
  const lockPath = hubDatabaseLockPath(resolvedDatabasePath);
  mkdirSync(dirname(lockPath), { recursive: true });

  for (let attempt = 0; attempt < 2; attempt += 1) {
    const record: HubDatabaseLockRecord = {
      pid: process.pid,
    ...
    try {
      ...
      if (current?.pid && processIsRunning(current.pid)) {
        throw new HubDatabaseLockedError(lockPath, current);
      }
      if (!current && lockFileIsFresh(lockPath)) {
        throw new HubDatabaseLockedError(lockPath, null);
      }
      rmSync(lockPath, { force: true });
    }
  }

  throw new HubDatabaseLockedError(lockPath, readLockRecord(lockPath));
}

export function ensureHubInstanceId(
  db: DatabaseSync,
  options: { now?: string; instanceId?: string } = {},
): string {
  const existing = readHubInstanceId(db);
  if (existing) return existing;

  const instanceId = options.instanceId ?? `hub_${randomUUID().replaceAll("-", "")}`;
  if (!/^hub_[a-f0-9]{32}$/u.test(instanceId)) {
    throw new Error("Hub instance id must match hub_<32 lowercase hex characters>.");
  }
  db.prepare(
    ...
    hubInstanceIdSettingKey,
    JSON.stringify(instanceId),
    options.now ?? new Date().toISOString(),
  );
  return readHubInstanceId(db) ?? instanceId;
}
...
export async function createHubSnapshot(input: {
  sourceDb: DatabaseSync;
  destination: string;
  now?: string;
}): Promise<HubSnapshotManifest> {
  const destination = resolve(input.destination);
  if (existsSync(destination)) {
    throw new Error(`Hub snapshot destination already exists: ${destination}`);
  }
  ...
  try {
    ...
    const manifest: HubSnapshotManifest = {
      format: hubSnapshotFormat,
...
export function inspectHubSnapshot(snapshotPath: string): HubSnapshotInspection {
  ...
  if (!existsSync(databasePath)) {
    throw new HubSnapshotValidationError(`Snapshot database is missing: ${databasePath}`);
  }

  const sizeBytes = statSync(databasePath).size;
  if (sizeBytes !== manifest.database.sizeBytes) {
    throw new HubSnapshotValidationError(
  ...
  if (sha256 !== manifest.database.sha256) {
    throw new HubSnapshotValidationError("Snapshot database SHA-256 does not match manifest.");
  }

  const summary = inspectDatabaseFile(databasePath);
  assertCompatibleMigrations(summary.schemaMigrations);
  if (summary.instanceId !== manifest.instanceId) {
    throw new HubSnapshotValidationError("Snapshot instance id does not match manifest.");
  }
  if (JSON.stringify(summary.schemaMigrations) !== JSON.stringify(manifest.schemaMigrations)) {
    throw new HubSnapshotValidationError("Snapshot migration list does not match manifest.");
  }
  if (JSON.stringify(summary.tableCounts) !== JSON.stringify(manifest.tableCounts)) {
    throw new HubSnapshotValidationError("Snapshot table counts do not match manifest.");
  }
...
export async function restoreHubSnapshot(input: {
  snapshotPath: string;
  databasePath: string;
  rollbackRoot?: string;
  now?: string;
...
}): Promise<HubRestoreResult> {
  const inspection = inspectHubSnapshot(input.snapshotPath);
  const databasePath = resolve(input.databasePath);
  const lock = acquireHubDatabaseLock(databasePath);
    ...
    if (targetExisted) {
      ...
      try {
        ...
        rollbackSnapshotPath = uniqueSnapshotPath(
          rollbackRoot,
          `hub-before-restore-${compactTimestamp(now)}`,
        );
        await createHubSnapshot({ sourceDb: targetDb, destination: rollbackSnapshotPath, now });
      } finally {
        targetDb.close();
      }
    }
...
function prepareRestoredDatabase(
  databasePath: string,
  now: string,
): HubRestoreResult["transientReset"] {
  ...
  try {
      ...
      db
        .prepare(
          `UPDATE runtime_sessions
           SET status = 'closed', closed_at = COALESCE(closed_at, ?), close_reason = 'hub_relocated'
           WHERE status != 'closed'`,
        )
        .run(now),
    );
    ...
    const webPushSubscriptionsDeleted = changes(
      db.prepare("DELETE FROM app_settings WHERE key = ?").run(hubWebPushSubscriptionSettingKey),
    );
    db.exec("COMMIT");
...
function inspectPreparedDatabase(databasePath: string, expectedInstanceId: string): void {
  ...
  if (summary.instanceId !== expectedInstanceId) {
    throw new HubSnapshotValidationError("Prepared database changed Hub instance id.");
  }
}
...
function inspectDatabaseFile(databasePath: string): {
  ...
  try {
    ...
    if (integrity.length !== 1 || integrity[0]?.integrity_check !== "ok") {
      throw new HubSnapshotValidationError("Snapshot database failed SQLite integrity_check.");
    }
    ...
    const schemaMigrations = db
      ...
      .all() as unknown as HubSnapshotMigration[];
    const tables = db
      .prepare(
        `SELECT name
         FROM sqlite_master
    ...
    return { instanceId: readHubInstanceId(db), schemaMigrations, tableCounts };
  } catch (error) {
    if (error instanceof HubSnapshotValidationError) throw error;
    throw new HubSnapshotValidationError(
...
function assertCompatibleMigrations(snapshotMigrations: HubSnapshotMigration[]): void {
  const current = loadMigrations().map(({ version, name }) => ({ version, name }));
  if (snapshotMigrations.length === 0) {
    throw new HubSnapshotValidationError("Snapshot has no recorded schema migrations.");
  }
  ...
  if (snapshotMigrations.length > current.length) {
    throw new HubSnapshotValidationError("Snapshot schema is newer than this Spark build.");
  }
}

function parseSnapshotManifest(raw: string): HubSnapshotManifest {
  let value: unknown;
  try {
  ...
  if (
    ...
    throw new HubSnapshotValidationError("Snapshot manifest shape is invalid.");
  }
  ...
  const schemaMigrations = value.schemaMigrations.map((entry) => {
    if (!isRecord(entry) || typeof entry.version !== "string" || typeof entry.name !== "string") {
      throw new HubSnapshotValidationError("Snapshot manifest migration entry is invalid.");
    }
  ...
  for (const [key, count] of Object.entries(value.tableCounts)) {
    if (!Number.isInteger(count) || (count as number) < 0) {
      throw new HubSnapshotValidationError(`Snapshot table count is invalid for ${key}.`);
    }
...
function stringArray(value: unknown[], field: string): string[] {
  if (!value.every((entry): entry is string => typeof entry === "string" && entry.length > 0)) {
    throw new HubSnapshotValidationError(`Snapshot manifest ${field} is invalid.`);
  }
  return value;
}
...
function readLockRecord(path: string): HubDatabaseLockRecord | null {
