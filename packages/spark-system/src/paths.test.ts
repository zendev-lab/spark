...
describe("Spark path resolution", () => {
  it("uses standard XDG defaults when SPARK_HOME is unset", () => {
    ...
    expect(user).toMatchObject({
      ...
      memoryDir: join(home, ".local", "share", "spark", "memory"),
      learningsDir: join(home, ".local", "share", "spark", "memory", "learnings"),
      recallFile: join(home, ".local", "share", "spark", "memory", "recall-candidates.json"),
