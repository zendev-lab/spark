...
Inbound messages normalize to `IncomingMessage` with canonical `externalKey`
values; a consuming host owns session bind/resolve and assignment.
...
The surface follows pi-channels (`adapters` / `routes` / `notify` / ingress).
