...
- `plan`, `execute`, and `fleet` Spark Session lenses. Investigation and research are activities within `plan`, not separate lenses; Spark registers Fleet through its product composition layer.
...
`spark-modes` is a host-neutral mechanism package: “mode” means any registered lens id. Spark’s durable Session operating axis is `plan | execute | fleet`, so the Spark native tool and specs use the name **`mode`** (`mode({ action })`). Hosts pass `createModeTool({ name: "mode", label: "Mode" })` and Spark's composition layer registers Fleet policy.
