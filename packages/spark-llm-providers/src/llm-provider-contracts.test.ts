...
test("spark-llm-providers validates a complete SparkModelProfile", () => {
...
test("spark-llm-providers registry stores validated profiles and rejects duplicate ids", () => {
...
test("spark-llm-providers rejects profiles without routes", () => {
  assert.throws(
...
test("spark-llm-providers rejects routes without a transport API", () => {
  const profile = sampleProfile({
    routes: [
      {
        id: "baidu-anthropic-primary",
...
test("spark-llm-providers rejects routes when auth pools are absent or empty", () => {
...
test("spark-llm-providers rejects duplicate route ids and unknown auth pools", () => {
  const profile = sampleProfile({
    routes: [
      {
        id: "duplicate-route",
...
test("spark-llm-providers rejects malformed auth slots without exposing secrets", () => {
  const profile = sampleProfile({
    authPools: [
      {
        id: "baidu-primary",
