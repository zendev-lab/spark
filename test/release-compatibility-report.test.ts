...
function productReport(value: Record<string, any>) {
...
function databaseReport(value: Record<string, any>) {
  return {
    schemaVersion: 1,
    contractSchemaVersion: value.schemaVersion,
    candidateVersion: "0.3.0",
    ...
    owners: value.database.owners.map((owner: { id: string; migrationManifest: string }) => ({
      ...
      candidateHead: owner.id === "daemon" ? "legacy-inline-v0" : "0022",
      baselineHead: "legacy",
...
test("rejects missing, duplicate, failed, and unverifiably cleaned product phases", async () => {
  ...
  for (const mutate of [
    (report: any) => report.phases.pop(),
    (report: any) => report.phases.push(structuredClone(report.phases[0])),
    (report: any) => (report.phases.at(-1).assertions[0].status = "failed"),
    (report: any) => (report.phases.at(-1).cleanup.status = "unverified"),
  ]) {
...
test("rejects incomplete database owner, phase, assertion, and cleanup evidence", async () => {
  ...
  for (const mutate of [
    ...
    (report: any) => (report.owners[0].phases[0].assertions = []),
