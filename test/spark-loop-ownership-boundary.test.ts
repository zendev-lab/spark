...
test("Spark extension adapters do not own autonomous Loop timers or turn state", async () => {
  ...
  for (const [path, text] of [
    ...
    assert.doesNotMatch(text, /\bsetTimeout\s*\(/u, `${path} must not own a Loop timer`);
    assert.doesNotMatch(
      text,
      /SparkForegroundDriveSubstrate|awaitingTurn|foregroundGeneration/u,
      `${path} must not retain frontend Loop state`,
    );
  }
  assert.doesNotMatch(
    commandRegistration,
    /["']agent_end["']|["']turn_end["']|["']tool_execution_end["']/u,
    "Loop continuation must not be attached to Pi lifecycle events",
  );
