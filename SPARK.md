...
- **执行宿主**：`spark-host` + `spark-turn` 服务 TUI / headless / daemon；`apps/spark-daemon` 拥有会话、通道与 SQLite；`apps/spark-tui` 与 `apps/spark-hub` 是一等产品面。
...
- **能力包**：`spark-ask`、`spark-artifacts`、`spark-tasks`、`spark-roles`、`spark-cue`、`spark-channels`、`spark-hub-coordination` 等；工具表面使用规范化 `tool({ action })`。
...
- 会话队列双层收敛：TUI 乐观层 ↔ daemon `pendingTurns` 真相；Hub 继续只投影 daemon。
...
- Hub v1 先留在现有 `spark-hub-coordination` / `spark-hub-db` owner 内：完成逻辑 Hub CLI、workspace 主 session、委托协议/状态机/outbox、Hub 委托视图和旧 Hub CLI 迁移提示；物理 package/数据库目录改名作为独立迁移。
